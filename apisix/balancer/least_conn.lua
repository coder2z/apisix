...
local function get_server_conn_count(upstream, server)
    if not conn_count_dict then
        return 0  -- Fallback when shared dict is not available
    end
...
local function cleanup_stale_conn_counts(upstream, current_servers)
    if not conn_count_dict then
        return  -- Skip cleanup when shared dict is not available
    end
...
function _M.new(up_nodes, upstream)
    ...
    if not conn_count_dict then
        core.log.warn("shared dict '", CONN_COUNT_DICT_NAME, "' not found, ",
                     "falling back to basic least_conn without persistent connection tracking")
    end
    ...
    for server, weight in pairs(up_nodes) do
        -- Get the persisted connection count for this server (returns 0 if shared dict unavailable)
        local conn_count = get_server_conn_count(upstream, server)
        -- Score calculation: when shared dict is unavailable, conn_count will be 0,
        -- so this falls back to the original behavior: score = 1 / weight
        ...
        core.log.debug("initializing server ", server,
                ...
                " | shared_dict_available: ", conn_count_dict and "yes" or "no")

        -- Note: the argument order of insert is different from others
        servers_heap:insert({
            server = server,
            weight = weight,
            effect_weight = 1 / weight,  -- Keep for backward compatibility
            score = score,
        }, server)
    end
    ...
    return {
        ...
        get = function(ctx)
            ...
            if conn_count_dict then
                -- Use persistent connection tracking when shared dict is available
                ...
                -- Fallback to original logic when shared dict is not available
